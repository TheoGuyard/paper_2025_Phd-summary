...
\author{Théo Guyard \\ INRIA, Centre de l'Université de Rennes, Rennes, France \\ INSA Rennes, IRMAR -- UMR CNRS 6625, Rennes France}
\date{\today}
...
\begin{equation}
    ...
    \tag{$\ppb$}
    \textstyle
    \opt{\pobj} = \min_{\pv \in \kR^{\pdim}} \lfunc(\pv) + \rfunc(\pv)
    \quad\text{with}\quad
    \rfunc(\pv) = \reg\norm{\pv}{0} + \pfunc(\pv)
\end{equation}
where $\kfuncdef{\lfunc}{\kR^{\pdim}}{\kR \cup \{+\infty\}}$ is a loss function and $\kfuncdef{\rfunc}{\kR^{\pdim}}{\kR \cup \{+\infty\}}$ is a regularization involving an $\ell_0$-norm defined as $\norm{\pv}{0} = \mathrm{card}{\kset{\idxentry}{\pvi{\idxentry} \neq 0}}$ with weight $\reg > 0$ which counts the number of non-zeros in its argument to promote sparse solutions, as well as another penalty term $\kfuncdef{\pfunc}{\kR^{\pdim}}{\kR \cup \{+\infty\}}$ used to promote other desired properties than sparsity.
...
So far, these methods have only been designed for instances of problem \eqref{prob:prob} with a specific expression of penalty $\pfunc$ and can still suffer from poor numerical performance on real-world data.
...
\Cref{fig:perfprofiles} gives a glimpse of the implications of the manuscript's contributions for a selected application in signal processing.
Here, we consider Bernoulli-Mixture inverse problems \cite{soussen2011bernoulli} aiming to reconstruct a sparse signal $\groundtruth \in \kR^{1000}$ sampled as $\groundtruthi{\idxentry} \sim \mathcal{D}$ with probability $\density = 0.01$ for some distribution $\mathcal{D}$, and fixed to $\groundtruthi{\idxentry}=0$ otherwise. 
...
To conduct our experiment, we select different choices of distributions (Normal, Gauss-Laplace, Half-Normal, Exponential), generate 100 independent instances of the corresponding problem \eqref{prob:prob}, and represent the percentage solved within a given time budget using different numerical procedures.
When compared to generic solvers such as \texttt{Cplex} and \texttt{Mosek} \cite{kronqvist2019review}, but also to solution methods specifically designed for problem \eqref{prob:prob} such as \texttt{L0bnb} \cite{hazimeh2022sparse} and \texttt{OA} \cite{bertsimas2021unified}, our proposed solver \texttt{El0ps} allows for more flexibility in the instances it can handle.
Notably, it is the only method able to process instances associated with the Exponential distribution.
Additionally, it largely outperforms its competitors in terms of numerical performance on instances that could already been addressed.
